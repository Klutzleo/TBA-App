...
async def broadcast_achievement_awarded(campaign_id: UUID, user_id, newly_awarded: list[str], db: Session):
    ...
    for aid in newly_awarded:
        ...
        if not meta:
            continue
        scope = meta.get("broadcast", "personal")
        payload = {
...
async def broadcast_bap_retroactive(
    ...
    defender_new_dp: int = None, narrative: str = "", defender_old_dp: int = None
):
    ...
    await manager.broadcast(campaign_id, {
        ...
        "defender_old_dp": defender_old_dp,
        "narrative": narrative
    })
...
async def cancel_holding_triple_combo_on_damage(campaign_id: UUID, character_id, db: Session):
    ...
    await manager.broadcast(campaign_id, {
        ...
        "proposer_character_id": str(combo.proposer_character_id),
        "partner_a_character_id": str(combo.partner_a_character_id),
        "partner_b_character_id": str(combo.partner_b_character_id),
...
async def _handle_triple_combo_accept(campaign_uuid: UUID, user_uuid: UUID, data: dict, websocket, db: Session):
    ...
    await manager.broadcast(campaign_uuid, {
        ...
        "combo_id": str(combo.id),
        "proposer_character_id": str(combo.proposer_character_id),
        "non_last_acceptor_character_id": str(other_char.id) if other_char else None,
